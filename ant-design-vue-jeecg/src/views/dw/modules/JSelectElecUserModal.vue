    ...
    methods: {
      initUserNames() {
        if (this.userIds) {
          ...
          console.log(values)
          getAction('/userinfob/userInfoB/list',{
            username: values[0],
          }).then((res) => {
            if (res.success) {
      ...
      async loadData(arg) {
        ...
        if (this.selectedDepIds && this.selectedDepIds.length > 0) {
          ...
          await getAction('/userinfob/userInfoB/list',params).then((res) => {
            if (res.success) {
      ...
      getSelectUserRows(rowId) {
        ...
        for (let i = 0, len = dataSource.length; i < len; i++) {
          if (this.selectedRowKeys.includes(dataSource[i].id)) {
            ...
            userIds = userIds + "," + dataSource[i].comAName
          }
        }
        console.log(userIds)
